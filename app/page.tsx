		...
		<div className='flex flex-col'>
			...
			<section className='relative flex flex-col items-center justify-center px-4 pb-16 pt-20 text-center md:pb-24 md:pt-32'>
				...
				<div className='relative flex flex-col items-center gap-6'>
					...
					<h1 className='max-w-3xl text-balance text-4xl font-bold tracking-tight md:text-6xl lg:text-7xl'>
						How well do you <span className='text-primary'>know</span> your music?
					</h1>

					<p className='max-w-xl text-pretty text-base text-muted-foreground md:text-lg'>Listen to audio snippets, watch video clips, or read lyrics and guess the song. Challenge yourself across three unique game modes.</p>
			...
			<section className='mx-auto w-full max-w-6xl px-4 pb-20'>
				...
				<div className='grid gap-4 md:grid-cols-3'>
					{gameModes.map((mode) => (
						<Link key={mode.title} href='/play' className={`group flex flex-col gap-4 rounded-xl border ${mode.borderColor} ${mode.bgColor} p-6 transition-all hover:scale-[1.02] hover:shadow-lg`}>
			...
			<section className='flex flex-col items-center gap-6 px-4 py-20 text-center'>
				...
				<p className='max-w-md text-muted-foreground'>Sign up to save your scores, track your progress, and compete on the global leaderboard.</p>
			...
			<footer className='border-t border-border/50 px-4 py-6'>
				<div className='mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 text-sm text-muted-foreground md:flex-row'>
					<p>SimOne - A music guessing game</p>
