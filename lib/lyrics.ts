...
export async function searchLyrics(query: string): Promise<LyricsData | null> {
	try {
		const res = await fetch(`https://lrclib.net/api/search?q=${encodeURIComponent(query)}`, {
			headers: {
				'User-Agent': 'SimOne/1.0 (https://SimOne.app)',
			},
		});
		...
		const match = results.find((r) => r.plainLyrics && !r.instrumental);
...
export function getLyricsForAttempt(lines: string[], attemptNumber: number, maxAttempts: number = 6): { visible: string[]; hidden: number } {
	...
	const visibleCount = Math.max(1, Math.ceil(lines.length * fraction * 0.8));
